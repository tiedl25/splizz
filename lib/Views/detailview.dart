...
class _DetailViewState extends State<DetailView> {
  ...
  Future<void> _showShareDialog() async {
    User permission = await DatabaseHelper.instance
        .getPermission(item.id, Supabase.instance.client.auth.currentUser!.id);
  ...
  List<Container> memberBar() {
    return List.generate(item.members.length, (index) {
      Member m = item.members[index];
      Color textColor =
          Color(m.color).computeLuminance() > 0.2 ? Colors.black : Colors.white;

      return Container(
          foregroundDecoration: !m.active
              ? const BoxDecoration(
                    ...
                    children: [
                      Icon(
                          ...
                          color: m.balance >= 0
                              ? Colors.green[700]
                              : Colors.red[700]),
                      Text('${m.balance.abs().toStringAsFixed(2)}€',
                          style: TextStyle(
                              fontSize: 20,
                              color: m.balance >= 0
                                  ? Colors.green[700]
                  ...
                  if (unbalanced) {
                    showDialog(
                      ...
                      builder: (BuildContext context) {
                        return PayoffDialog(
                            item: item,
                            updateItem: (data) => setState(() => item = data));
                      },
                    );
                  }
                },
      ...
      child: Container(
          ...
          child: RefreshIndicator(
            onRefresh: () {
              setState(() {
                itemFuture =
                    DatabaseHelper.instance.getItem(widget.item.id, sync: true);
              });
              return itemFuture;
            },
  ...
  Widget dismissibleTile(
      Transaction transaction, Map<String, int> memberMap, int index) {
    return Container(
      margin: const EdgeInsets.only(bottom: 5),
      decoration: const BoxDecoration(
        borderRadius: BorderRadius.all(Radius.circular(20)),
            ...
            child: Container(
              ...
              child: Row(
                children: List.generate(transaction.operations.length, (index) {
                  ...
                  Member m = item.members[
                      memberMap[transaction.operations[index].memberId]!];
                  return Container(
                    padding: const EdgeInsets.all(5),
                    margin: const EdgeInsets.all(2),
                    decoration: BoxDecoration(
  ...
  Widget body() {
    double imageRadius = window.viewPadding.top -
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
              onPressed: _showShareDialog, icon: const Icon(Icons.share)),
        ],
      ),
      body: body(),
                      ...
                      List<Map<String, dynamic>> involvedMembers =
                        ...
                        return {
                          ...
                          'balance': double.parse(
                              (22.00 / item.members.length).toStringAsFixed(2))
                        };
                      }).toList();
                      setState(() {
                        Transaction t = Transaction(
                            ...
                            value: 22.00,
                            date: DateTime.now(),
                            memberId: item.members[memberListIndex].id,
                            itemId: item.id);
                        item.addTransaction(
                            memberListIndex, t, involvedMembers);

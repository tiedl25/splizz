...
class PayoffDialog extends StatefulWidget {
  ...
  const PayoffDialog({Key? key, required this.item, required this.updateItem})
      : super(key: key);
...
class PastPayoffDialog extends StatefulWidget {
  ...
  const PastPayoffDialog({Key? key, required this.item, required this.index})
      : super(key: key);
...
class _PastPayoffDialogState extends State<PastPayoffDialog> {
  ...
  Future<File> widgetToImageFile(
      Uint8List capturedImage, String filename) async {
        ...
        header: Row(children: [
          ...
          IconButton(
              onPressed: () async {
                ...
                final transactionsBytes =
                    await transactionTable(item.history[widget.index]);
                final excelBytes = await exportTransactionTableToExcel(
                    item.history[widget.index]);
